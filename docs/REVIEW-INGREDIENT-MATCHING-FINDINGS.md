...
| 3b | **Revised 23 Sep, after `docs/REVIEW-ARCHITECTURE-FINDINGS.md` §4 and F12.** The converter's rule to rename to the vocabulary is withdrawn: it swapped *Italian seasoning* for *dried oregano* on its first unfamiliar site, and the review showed the rule never earned the result (every measured gain came from the line shape and app-side rules; all 24 "converter-only" lines were shape faults). The converter keeps the source's product and ambiguities in British English; `ingredient-names.md` is now the **app's** dictionary, read by the validator and survey to report, not to fault. Test 7 reframed. The plan for steps 4–8 stands, with the fidelity check added (§9 of that review) |
| 4–6 | **Done: PR 6b, #19, merged Friday 25 Sep 2026.** `shoppingLine`, `aggregateShoppingLines` and `strictMatchSuggestions` in `core.js`: the naming rules of §3, the dictionary with aisles, one row per ingredient (`2 + 400 g`), spoons totalled as ml, ticks keyed by name (old keys deleted at start-up, M3), word matches re-normalised as they load with no data written, and the inline MERGE · KEEP APART in place of the `confirm()` questions. **Re-measured against the live library, read-only:** 185 rows library-wide against the review's 178 names; Week A 60 rows (the review's 60), Week B 54 (52 — the two extra are a cooked-weight line, D4, and an "X or Y" line, both 6c's to rewrite; the private appendix names them). The 6 "same" word matches all become redundant (M4); none of the "not the same" answers is re-offered. The strict suggester made 1 proposal on the whole library, a line 6c rewrites, after a rule the review didn't have: a pair is not offered when the dictionary knows either name as a different product (without it, 5 of 7 proposals were wrong, all a product the dictionary lists against a bare word — made up, *raspberry jam ~ jam*). §6 checks 7–21, 23 and 24 are in `test/core.test.js` and `test/smoke.js`, each run once against its mutation. D5 is settled by the pepper rule; D4 waits on 6c. Categories are not finished: 45 of 185 rows landed in Other (§2.7), 36 after the follow-up, PR #20, which also fixed three rows a keyword put in the wrong aisle. **Checked live on 25 Sep:** Week A planned into the app gave 60 rows, matching this measure row for row |
| 4–8 | **Split on 25 Sep into PRs 6a, 6b and 6c** (`docs/NEXT-SESSION.md`). **6a done, PR #18:** the dictionary's master copy is `INGREDIENT_DICTIONARY` in `core.js`, `converter/ingredient-names.md` is generated from it, and check 22 exists (`test/core.test.js` fails if they differ). Steps 4–6 are 6b; step 7 is 6c. Originally: not started; sequenced as PR 6 in `docs/NEXT-SESSION.md`, after row-scoped writes. **Step 4 must also:** make the app's dictionary the master copy, with `converter/ingredient-names.md` generated from it and a test that fails if they differ (§6, check 22); and add a private re-measure after every ten or so new recipes (§6, library regression), watching one number, *items split across names* |
