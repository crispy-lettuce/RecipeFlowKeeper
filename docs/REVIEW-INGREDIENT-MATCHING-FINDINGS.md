...
| 3a | **Guarding against over-fitting, 23 Sep.** The vocabulary is a small list, not the mechanism. The converter's first rule is now to *name an ingredient as a UK supermarket shelf labels it*, which works on ingredients no list has seen. The list keeps the household's decisions and the ambiguous words, and its spellings are kept as examples the validator checks against. `validate-recipes.js` now reports names the list doesn't know, so an entry is added when an ingredient turns up in a second recipe. Converter test 8 uses only unlisted ingredients. The tin rule, which had been fitted to chopped tomatoes, now reads "the unit on the tin" |
